...
function buildData(rows: MonthlyActualsRow[]): ChartRow[] {
  return rows.map((r) => ({
    ...
    budgeted: r.budgetedMonthlySpend,
    forecasted: r.forecastedMonthlySpend,
    actual: r.actualMonthlySpend,
  }))
}
...
export function MonthlySpendComparisonChart({ rows }: Props) {
  const data = buildData(rows)

  return (
    <div className="rounded-xl border border-zinc-200 bg-white p-5 print:break-inside-avoid">
      <h3 className="mb-1 text-sm font-semibold text-zinc-700">Monthly spend</h3>
      <p className="mb-4 text-xs text-zinc-500">
        Budgeted monthly spend (constant), forecasted monthly spend (budget minus forecast
        variance), and actual monthly spend when an actual variance is entered for that month.
      </p>
